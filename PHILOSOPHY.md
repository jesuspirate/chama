...
**Federation follows the listing, not the user.** A trade can only be locked when buyer, seller, and arbiter are all on the same federation — period, no exceptions, no probes, no warnings. Cross-federation locks would produce stranded ecash (mint on fed A cannot be redeemed on fed B), so they must be structurally impossible to attempt, not "warned against." When a user taps a listing in a different federation than their current wallet, Chama silently switches the client to match — but ONLY if OPFS balance is zero. If the user has non-zero balance, the UX is "drain wallet via QR-OUT first, then this listing becomes trade-able." The community is the user's *identity layer* (currency, region, language, browse filtering); the federation is *per-trade plumbing* derived automatically from the listing. Listings in non-matching federations remain visible (peek mode preserved per §2.3), but the action button changes from "Fund trade" to "Drain to switch and trade." Power-user federation tribalism is a v2 concern; v1 prioritizes structural correctness over user choice in this dimension.
