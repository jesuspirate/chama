...
const PORT = 20_099, MAXW = 40_000, ESCROW = 100_000n, W = 4;
...
async function main() {
  ...
  try {
    ...
    rec.report("Observer + bounded fee controller under congestion — Bitcoin Core " + (await rpc<any>("getnetworkinfo", [], "")).subversion, path.join(HERE, "controller-results.json"), { minRelaySatPerVb: minRelay, blockMaxWeight: MAXW, budgets: JSON.parse(fs.readFileSync(path.join(tmp, "budgets.json"), "utf8")) });
