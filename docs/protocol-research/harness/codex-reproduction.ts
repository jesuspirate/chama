// Codex instrumentation of Claude commit 1d7e57e. Transaction construction
// unchanged; records tip heights and negative mining calls on review ports.
// ─────────────────────────────────────────────────────────────────────────────
...
async function mine(n: number) {
  if (n < 0) console.warn(`REVIEW: negative mining request ${n} at tip ${await rpc("getblockcount")}`);
...
async function main() {
  ...
  try {
    ...
    fs.writeFileSync(path.join(path.dirname(new URL(import.meta.url).pathname), "codex-reproduction-results.json"), JSON.stringify(out, null, 2));
