...
async function main() {
  ...
  try {
    ...
    const p1 = new DeviceSponsorPool(path.join(tmp, "dev1.sqlite"), "device-1", hex(btc.p2tr(d1.xonly, undefined, REGTEST).script)), p2 = new DeviceSponsorPool(path.join(tmp, "dev2.sqlite"), "device-2", hex(btc.p2tr(d2.xonly, undefined, REGTEST).script));
    ...
    err = undefined; try { new DeviceSponsorPool(path.join(tmp, "dev2-copy.sqlite"), "device-3", hex(s2)); } catch (e) { err = e; }
    ...
    let cloneOpened = false; try { const clone = new DeviceSponsorPool(path.join(tmp, "dev2-copy.sqlite"), "device-2", hex(s2)); cloneOpened = true; clone.close(); } catch { /* */ }
    ...
    rec.report("Admission, bounded index, sponsor ownership — Bitcoin Core " + (await rpc<any>("getnetworkinfo", [], "")).subversion, path.join(HERE, "codex-aio-fixed-results.json"));
