...
async function main() {
  ...
  try {
    ...
    const policy = await rpc<any>("getnetworkinfo", [], "");
    const minRelay = policy.relayfee * 100000;
    ...
    // ── M2: attacker pins at a HIGH feerate. Honest cost tracks the attacker's own spend. ──
    {
      ...
      const pinV = await vsizeOf(pin);
      const pkg = await rpc<any>("submitpackage", [[parent, pin]]);
      rec.record("M2a", `attacker pins at ${(12000 / pinV).toFixed(1)} sat/vB with ${pinV} vB (attacker offers 12000 sat)`, "ACCEPT", pkg.package_msg === "success", JSON.stringify(pkg));
      await rec.probe("C1", "honest 2000-sat budget cannot evict the 12000-sat bid", anchorChild(ptxid, 1, honestSponsors[1], HONEST, 2_000n), "REJECT");
      let thr2 = 12_000n;
      ...
      await rec.probe("M2c", `honest replacement at the measured threshold (${thr2} sat = attacker fee + ${thr2 - 12_000n})`, honest, "ACCEPT", "replacement threshold does not measure attacker realized cost");
      await rpc("sendrawtransaction", [honest]); await mine(rpc, 1);
      const remaining = await rpc<any>("gettxout", [attackerSponsors[1].txid, attackerSponsors[1].vout]);
      rec.record("C2", "evicted attacker retains full sponsor; honest replacement confirmed", "ACCEPT",
        remaining?.value === 0.002 && (await confs(txidOf(honest))) === 1,
        JSON.stringify({ attackerSponsorUnspentSats: remaining?.value * 100000000, attackerConfirmedFeeSats: 0, honestConfirmedFeeSats: Number(thr2) }));
    }
    ...
    rec.report("Hostile fee matrix — Bitcoin Core " + (await rpc<any>("getnetworkinfo", [], "")).subversion,
      path.join(path.dirname(new URL(import.meta.url).pathname), "codex-fee-audit-results.json"),
