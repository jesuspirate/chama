...
If the wallet or Nostr relays are still starting, Chama keeps you signed in and preserves your selected home community. The Chama bar shows **Connecting** while startup is active and **Reconnect** if it fails. The browser Fedimint client is local to that device: Chama reopens it when its browser storage is intact and creates a new local client when that storage is genuinely absent. Wallet startup reuses one operation-history reading for payout repair and pending-payment resumption, and a device-local browser wallet does not wait for Nostr seed storage it does not use. A storage-handle retry always reopens the same wallet file; Chama does not replace a possibly funded wallet to make Reconnect appear successful. On iPhone, use standard Safari if a third-party or private browser reports that secure local wallet storage is unavailable. Its “unknown transient reason (e.g. out of memory)” message is a WebKit storage-operation error and does not establish that the phone itself lacks memory. Signing into an npub that has used Chama elsewhere does not launch a federation-recovery job. Move each completed trade out through Lightning, on-chain, or ecash; your Nostr account key restores your identity and trade history, not bearer ecash left in another browser's local wallet.
