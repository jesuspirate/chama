...
export function isPrivatePlanChild(payload: Pick<CreatePayload, "parent" | "tranche">): boolean {
  return payload.tranche?.privatePlanChild === true;
}
...
export function derivePlanState(parent: EscrowState, children: EscrowState[]): PlanState | null {
  ...
  const byIndex = new Map(children.filter(c => c.tranche?.planId === frozen.planId).map(c => [c.tranche!.index, c]));
